...
SYSTEM_CONFIG_PATH = "/etc/electrum-wlc.conf"
...
class SimpleConfig(object):
    ...
    def init_path(self):
        # Read electrum-wlc path in the command line configuration
        ...
        print_error( "electrum-wlc directory", self.path)

    def set_key(self, key, value, save = True):
        if not self.is_modifiable(key):
            print "Warning: not changing key '%s' because it is not modifiable" \
                  " (passed as command line option or defined in /etc/electrum-wlc.conf)"%key
            return
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-wlc.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-wlc.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-wlc.conf into user_config[]."""
