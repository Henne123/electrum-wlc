...
try:
    ...
    print "You need to have PyQT installed to run Electrum-WLC in graphical mode."
...
from electrum_frc.util import get_resource_path as rsrc
from electrum_frc.bitcoin import is_valid
...
import re
import time
from electrum_frc.wallet import Wallet, WalletStorage
import webbrowser
import history_widget
import receiving_widget
from electrum_frc import util
import datetime

from electrum_frc.version import ELECTRUM_VERSION as electrum_version
from electrum_frc.util import format_satoshis, age
...
class MiniWindow(QDialog):
    ...
    def __init__(self, actuator, expand_callback, config):
        ...
        self.address_input.setPlaceholderText(_("Enter a Worldleadcurrency address or contact"))
        ...
        self.setWindowTitle("Electrum-WLC")
    ...
    def set_balances(self, btc_balance):
        """Set the worldleadcurrency balance and update the amount label accordingly."""
        ...
        self.setWindowTitle("Electrum-WLC %s - %s %s" % (electrum_version, amount, unit))

    def amount_input_changed(self, amount_text):
        """Update the number of worldleadcurrencys displayed."""
    ...
    def create_quote_text(self, btc_balance):
        ...
        user has in worldleadcurrencys."""
    ...
    def check_button_status(self):
        """Check that the worldleadcurrency address is valid and that something
...
class BalanceLabel(QLabel):
    ...
    def set_balance_text(self, amount, unit, quote_text):
        """Set the amount of worldleadcurrencys in the gui."""
        if self.state == self.SHOW_CONNECTING:
...
class ReceivePopup(QDialog):
    ...
    def setup(self, address):
        label = QLabel(_("Copied your Worldleadcurrency address to the clipboard!"))
        ...
        self.setWindowTitle("Electrum-WLC - " + _("Receive Worldleadcurrency payment"))
...
class MiniActuator:
    ...
    sending/receiving worldleadcurrencys."""
    ...
    def send(self, address, amount, parent_window):
        """Send worldleadcurrencys to the target address."""
        ...
        if dest_address is None or not is_valid(dest_address):
            QMessageBox.warning(parent_window, _('Error'),
                _('Invalid Worldleadcurrency Address') + ':\n' + address, _('OK'))
            return False
