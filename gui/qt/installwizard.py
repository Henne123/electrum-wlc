...
from electrum_frc import Wallet, Wallet_2of2, Wallet_2of3
from electrum_frc import bitcoin
...
from electrum_frc.mnemonic import prepare_seed

MSG_ENTER_ANYTHING    = _("Please enter a wallet seed, a master public key, a list of Worldleadcurrency addresses, or a list of private keys")
...
class InstallWizard(QDialog):
    ...
    def __init__(self, config, network, storage):
        ...
        self.setWindowTitle('Electrum-WLC' + '  -  ' + os.path.basename(self.storage.path))
    ...
    def restore_or_create(self):
        vbox = QVBoxLayout()
        main_label = QLabel(_("Electrum-WLC could not find an existing wallet."))
        ...
        for i, (wtype,name) in enumerate(self.wallet_types):
            if not filter(lambda x:x[0]==wtype, electrum_frc.wallet.wallet_types):
                continue
    ...
    def waiting_dialog(self, task, msg= _("Electrum-WLC is generating your addresses, please wait.")):
        def target():
            task()
    ...
    def network_dialog(self):
        ...
        label = QLabel(_("Electrum-WLC communicates with remote servers to get information about your transactions and addresses. The servers all fulfil the same purpose only differing in hardware. In most cases you simply want to let Electrum-WLC pick one at random if you have a preference though feel free to select a server manually.") + "\n\n" \
    ...
    def run(self, action):
        ...
        if action == 'new':
            ...
            if wallet_type == 'multisig':
                ...
                hardware_wallets = map(lambda x:(x[1],x[2]), filter(lambda x:x[0]=='hardware', electrum_frc.wallet.wallet_types))
